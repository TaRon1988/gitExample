...
public class FontExam extends JApplet{
	...
	private Dimension dim;//특정 영역의 사각형의 폭과 높이 값을 관리할 수 있도록 도와주는 클래스
	...
	public void init() {
		...
		x = (dim.width/2)-(fm.stringWidth(message)/2);//폰트의 가로길이 값 처리용
		y = (dim.height/2)-(fm.getDescent()/2);//폰트의 세로길이 값 처리용
